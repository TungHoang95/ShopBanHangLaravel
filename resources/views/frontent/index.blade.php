...
<!-- main -->
		...
		<div id="colorlib-intro" class="colorlib-intro" style="background-image: url(images/banner2.jpg);"
					...
					<div class="col-md-3 text-center">
						<div class="product-entry">
                            <div class="product-img" style="background-image: url(/backend/img/{{ $row->img }});">
                                ...
                                @else
                                <p class="tag"><span class="new">Highlights</span></p>
                                @endif
								<div class="cart">
									<p>
							...
							<div class="desc">
                                <h3><a href="/product/{{$row->slug}}.html">{{ $row->product_name }}</a></h3>
                                @if($row->sale_price == 0)
                                <p class="price"><span>{{ number_format($row->price,0,',','.') }} đ</span></p>
                                @else
                                <p class="price"><span><strike>{{ number_format($row->price,0,',','.') }} đ</span></strike>  <span>{{ number_format($row->sale_price,0,',','.') }} đ</span></p>
                                @endif
							</div>
						</div>
					</div>
			...
			<div class="container">
					<div class="row">
						<div class="col-md-6 col-md-offset-3 text-center colorlib-heading">
							...
							<p>Những mẫu sản phẩm được sale hot nhất năm</p>
						</div>
					</div>
                        <div class="row">
                            @foreach($prd_sale as $row)
                            <div class="col-md-3 text-center">
                                <div class="product-entry">
                                    <div class="product-img" style="background-image: url(/backend/img/{{$row->img}});">
                                        <p class="tag"><span class="new">Sale</span></p>
                                    ...
                                    <div class="desc">
                                        <h3><a href="/product/{{$row->slug}}.html">{{ $row->product_name }}</a></h3>
                                        <p class="price"><strike><span>{{ number_format($row->price,0,',','.') }} đ</span></strike>  <span>{{ number_format($row->sale_price,0,',','.') }} đ</span></p>
                                    </div>
                                </div>
                            </div>
                            @endforeach
